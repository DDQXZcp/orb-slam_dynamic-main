...
def transform44(l):
    t = np.array(l[1:4])  # Position vector
    q = np.array(l[4:8])  # Quaternion [qx, qy, qz, qw]
    
    # Create a rotation object from the quaternion
    rotation = R.from_quat(q)
    
    # Convert the rotation object to a 4x4 transformation matrix
...
# def transform44_rotate(l, rotation):
#     """
#     Convert position and quaternion to a 4x4 matrix and apply rotation.
#     """
#     t = l[1:4]
#     q = R.from_quat(l[4:8])
#     # Combine the initial rotation with the provided one
#     q_rotated = rotation * q
#     # Extract the rotated quaternion
#     q_rotated = q_rotated.as_quat()
#     matrix = np.zeros((4, 4))
#     matrix[:3, :3] = R.from_quat(q_rotated).as_matrix()
#     matrix[:3, 3] = rotation.apply(t)  # Apply rotation to the position
#     matrix[3, 3] = 1.0
#     return matrix
...
def read_trajectory(filename, matrix=True):
    ...
    list_ok = [[float(v.strip()) for v in line.split(" ") if v.strip() != ""] for line in lines if len(line) > 0 and line[0] != "#"]
    list_ok = [l for l in list_ok if len(l) == 8 and not np.any(np.isnan(l))]

    if matrix:
        traj = {l[0]: transform44(l) for l in list_ok}
    else:
        traj = {l[0]: l[1:8] for l in list_ok}
    
    return traj

# def read_trajectory_rotate(filename, x=0, y=0, z=0):
#     """
#     Read a trajectory from a text file and apply rotation if specified. 
    
#     Input:
#     filename -- file to be read
#     matrix -- convert poses to 4x4 matrices
#     x, y, z -- rotation around x, y, z axes in degrees
...
#     rotation = R.from_euler('xyz', [x, y, z], degrees=True)

#     traj = {l[0]: transform44_rotate(l, rotation) for l in list_ok}
...
max_pairs = 10000
fixed_delta = 'store_true'
delta = 1.0
delta_unit = 's'
offset = 0.0
scale = 1.0

def compute_rpe(groundtruth_path, trajectory1_path, trajectory2_path):

    traj_gt = read_trajectory(groundtruth_path)
    traj_est1 = read_trajectory(trajectory1_path)
    traj_est2 = read_trajectory(trajectory2_path)

    # Find common start and end times
    start_time = max(min(traj_est1.keys()), min(traj_est2.keys()))
    end_time = min(max(traj_est1.keys()), max(traj_est2.keys()))

    print(f"Common start time: {start_time}")
    print(f"Common end time: {end_time}")

    # Trim trajectories to match the same start and end times
    traj_est1_trimmed = {stamp: pose for stamp, pose in traj_est1.items() if start_time <= stamp <= end_time}
    traj_est2_trimmed = {stamp: pose for stamp, pose in traj_est2.items() if start_time <= stamp <= end_time}

    
    result1 = evaluate_trajectory(traj_gt,
                                 traj_est1_trimmed,
                                 int(max_pairs),
                                 fixed_delta,
                                 float(delta),
    ...
    trans_error1 = np.array(result1)[:,4]
    trans_error2 = np.array(result2)[:,4]

    return (np.mean(trans_error1)), (np.mean(trans_error2)), len(result1), len(result2)